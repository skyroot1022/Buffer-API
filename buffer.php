<?php 
	...
	class BufferApp {
		...
		function __construct($client_id = '', $client_secret = '', $access_token='', $callback_url = '') {
		...
		function go($param_method='post', $endpoint = '', $data = '') {
			...
			// 		}
			// 	}
			// 	echo "done_endpoint:".$done_endpoint."<br>";
			// 	if (!$ok) return $this->error('invalid-endpoint');
			// }
			
			if (!$data || !is_array($data)) $data = array();
			$data['access_token'] = $this->access_token;			
			$method = $param_method;//$this->endpoints[$done_endpoint]; //get() or post()
